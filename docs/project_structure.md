...
This chapter covers the five major top-level directories of the sdd-forge source tree: `src/presets` (preset definitions, data sources, and tests for each supported framework and technology), `src/docs` (documentation generation commands and processing libraries), `src/flow` (SDD workflow controller and subcommands), `src/lib` (shared utility libraries), and `src` (top-level CLI entry points).
<!-- {{/text}} -->
...
| --- | --- | --- |
| ProjectSource | `src/docs/data/project.js` | DataSource that reads `package.json` and exposes project metadata — `name`, `description`, `version`, and a Markdown table of npm `scripts` — as `{{data}}` directives. Caches the parsed package file on first access. |
<!-- {{/text}} -->
