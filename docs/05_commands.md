...
`sdd-forge <サブコマンド>` 形式で呼び出す 16 個のコマンドを収録しています。コマンド共通のグローバルオプションは `--help` / `-h` のみで、その他のオプションは各コマンドに固有です。
...
| コマンド | 説明 | 主なオプション |
|---|---|---|
| `help` | コマンド一覧・使い方を表示 | なし |
| `setup` | プロジェクト登録と初期設定ウィザード | `--name` `--path` `--type` `--agent` `--lang` `--dry-run` |
| `default` | デフォルトプロジェクトの確認・変更 | `<name>` |
| `build` | ドキュメント生成パイプラインを一括実行 | `--force` `--agent` |
| `scan` | ソースコードを解析し `analysis.json` を生成 | `--stdout` `--dry-run` `--legacy` |
| `init` | テンプレートから `docs/` 章ファイルを初期化 | `--type` `--force` `--dry-run` |
| `data` | `@data` ディレクティブを解析データで解決 | `--dry-run` `--stdout` |
| `text` | `@text` ディレクティブを AI エージェントで解決 | `--agent` `--id` `--per-directive` `--dry-run` |
| `readme` | `docs/` 章から `README.md` を自動生成 | `--dry-run` |
| `forge` | AI による `docs/` の反復改善 | `--prompt` `--spec` `--max-runs` `--mode` `--dry-run` |
| `review` | `docs/` の品質チェック（PASS / FAIL） | `<docs-path>` |
...
| `gate` | `spec.md` の実装前チェック（PASS / FAIL） | `--spec` |
| `flow` | SDD ワークフローを自動実行 | `--request` `--title` `--spec` `--agent` `--dry-run` |
...
| オプション | 説明 |
|---|---|
| `-h`, `--help` | コマンドの使い方とオプション一覧を表示して終了します。サブコマンドなし・`help` サブコマンド・このフラグのいずれでも呼び出せます。 |

`--dry-run` は多くのコマンドで共通して使用できますが、サポートの有無はコマンドごとに異なります。詳細は各コマンドのオプション表を参照してください。

### 各コマンドの詳細

<!-- @text: 各コマンドの使用方法・オプション・実行例を詳しく記述してください。コマンドごとにサブセクションを立てること。 -->

各サブコマンドの使用方法・オプション・実行例を以下のサブセクションにまとめています。ドキュメント生成系（`build` / `scan` / `init` / `data` / `text` / `readme` / `forge` / `review`）、仕様管理系（`spec` / `gate`）、ワークフロー系（`flow`）の順に記載しています。
...
|---|---|---|
| `0` | 正常終了（PASS） | 全コマンド |
| `1` | エラー・チェック失敗（FAIL） | `gate`・`review`・その他エラー時 |
| `2` | ユーザー入力待ち（NEEDS_INPUT） | `flow`（gate FAIL 時） |

**stdout / stderr の使い分け**

| チャンネル | 用途 |
|---|---|
| stdout | コマンドの主出力（生成テキスト・JSON・PASS/FAIL 判定結果・`--dry-run` のプレビュー） |
| stderr | 進捗メッセージ・警告・エラー詳細（スクリプトから stdout をパイプで受け取る際に分離できます） |

`--dry-run` を付与した場合、ファイルへの書き込みは行われず、変更内容が stdout へ出力されます。スクリプトから sdd-forge を呼び出す場合は終了コードでの判定を推奨します。
