...
本章では `sdd-forge` が提供する 16 のサブコマンドすべての仕様を解説します。各コマンドは `sdd-forge <subcommand>` の形式で呼び出し、`--help` / `-h` はどのコマンドでも共通して使用できます。コマンド群はドキュメント生成系（`build` / `scan` / `init` / `data` / `text` / `readme` / `forge` / `review` / `changelog` / `agents`）・スペック系（`spec` / `gate`）・フロー系（`flow`）・ユーティリティ系（`help` / `setup` / `default`）の 4 グループで構成されています。
...
|---|---|---|
| `help` | コマンド一覧・ヘルプを表示 | ─ |
| `setup` | プロジェクトの登録と初期設定（インタラクティブウィザード） | `--name` `--path` `--type` `--agent` `--lang` `--dry-run` |
| `default` | 登録済みプロジェクトの一覧表示・デフォルト変更 | `<name>` |
| `build` | ドキュメント生成パイプラインを一括実行 | `--force` `--agent` |
| `scan` | ソースコードを解析し `analysis.json` を出力 | `--stdout` `--dry-run` `--legacy` |
| `init` | テンプレートから `docs/` 章ファイルを初期化 | `--type` `--force` `--dry-run` |
| `data` | `@data` ディレクティブを解析データで解決・置換 | `--dry-run` `--stdout` |
| `text` | `@text` ディレクティブを AI エージェントで解決・置換 | `--agent` `--id` `--per-directive` `--dry-run` |
| `readme` | `docs/` の章ファイルから `README.md` を自動生成 | `--dry-run` |
| `forge` | AI エージェントによる `docs/` の反復改善 | `--prompt` `--spec` `--max-runs` `--mode` `--agent` `--dry-run` |
| `review` | `docs/` の品質チェックを実行 | `<path>` |
| `changelog` | `specs/` から変更履歴ファイルを生成 | `<output-file>` `--dry-run` |
| `agents` | `AGENTS.md` の PROJECT セクションを更新 | `--template` `--force` |
| `spec` | 機能仕様の spec ファイルと feature ブランチを作成 | `--title` `--base` `--no-branch` `--worktree` `--dry-run` |
| `gate` | `spec.md` の実装前チェックを実行 | `--spec` |
| `flow` | SDD ワークフローを自動実行 | `--request` `--title` `--spec` `--agent` `--no-branch` `--dry-run` |
...
| オプション | 説明 |
|---|---|
| `-h`, `--help` | ヘルプを表示して終了します。すべてのサブコマンドで使用できます。 |
| `--dry-run` | ファイルへの書き込みや外部エージェントの呼び出しを行わず、実行内容のみを表示します。対応しているコマンドで共通して使用できます。 |

### 各コマンドの詳細

<!-- @text: 各コマンドの使用方法・オプション・実行例を詳しく記述してください。コマンドごとにサブセクションを立てること。 -->

各サブコマンドの使用方法・オプション・実行例を以下に示します。コマンドは `sdd-forge <subcommand> [options]` の形式で呼び出します。`--dry-run` に対応しているコマンドは、本番実行前に挙動を確認することを推奨します。
...
| コード | 意味 | 該当コマンド |
|---|---|---|
| `0` | 正常終了・チェック PASS | すべてのコマンド |
| `1` | エラー終了・チェック FAIL | `gate`・`review`・その他エラー発生時 |
| `2` | 入力待ち停止（`NEEDS_INPUT`） | `flow`（gate FAIL 時） |

**stdout / stderr の使い分け**

| 出力先 | 内容 |
|---|---|
| stdout | コマンドの主出力（生成テキスト・JSON・`--dry-run` の差分表示・チェック結果サマリー） |
| stderr | 進行状況メッセージ・警告・エラーメッセージ |

`--dry-run` オプションを指定した場合、ファイルへの書き込みは行わず変更内容を stdout へ出力します。パイプやリダイレクトで利用する際は stdout のみを対象とし、stderr は無視してください。
