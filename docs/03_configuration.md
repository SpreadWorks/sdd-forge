...
| `textFill.projectContext` | string | `""` | `@text-fill` ディレクティブ解決時にAIへ渡すプロジェクト説明文 |
...
`.sdd-forge/project-overrides.json` にテーブル説明・コントローラ説明などを記述することで、ソースコード静的解析では取得できない意味情報を `@data-fill` / `@text-fill` ディレクティブへ補完できる。
