...
describe("estimateRelevantFiles", () => {
  ...
  it("returns matching files when spec mentions chapter keywords", () => {
    ...
    assert.ok(result.includes("docs/02_cli_commands.md"));
    assert.ok(result.includes("docs/03_configuration.md"));
    assert.ok(!result.includes("docs/04_internal_design.md"));
  });
