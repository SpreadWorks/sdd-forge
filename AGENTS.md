...
<!-- {{data: agents.project("")}} -->
以下が精査・改善した PROJECT セクションです。

---

## Project Context

- **generated_at:** 2026-03-11 08:23:46 UTC
...
| 外部依存 | **なし**（Node.js 組み込みモジュールのみ: `fs`, `path`, `child_process`, `os`, `crypto` 等） |
...
| テストファイル数 | 44 ファイル |
| 蓄積 spec 数 | 42 件（`specs/` ディレクトリ数） |

### プロジェクト自身の設定（`.sdd-forge/config.json`）

| フィールド | 値 |
|---|---|
| `lang` | `"ja"`（CLI・AGENTS.md の動作言語） |
| `output.languages` | `["en", "ja"]` |
| `output.default` | `"en"` |
| `type` | `"cli/node-cli"` |
| `documentStyle.purpose` | `"user-guide"` |
| `documentStyle.tone` | `"polite"` |
| `flow.merge` | `"squash"` |
| `defaultAgent` | `"claude"` |
| `providers.claude` | `claude --model sonnet -p {{PROMPT}}` |
...
├── package.json
├── src/
│   ├── sdd-forge.js              ← CLI エントリポイント（サブコマンドルーティング・プロジェクトコンテキスト解決）
...
│   │   │                            snapshot, upgrade, translate, enrich（15 ファイル）
...
│   │   │       │                    AGENTS.sdd.md
│   │   │       └── en/           ← （同上）
│   │   ├── webapp/, cli/, library/  ← アーキ層プリセット（isArch=true）
...
│   └── ja/                       ← 日本語版
├── tests/                        ← テストファイル（*.test.js、44 ファイル）
└── specs/                        ← SDD spec ファイル（42 件蓄積）
```
...
sdd-forge.js（エントリポイント）
  ...
  │                               enrich,default-project}.js（15 コマンド）
...
**プロジェクトコンテキスト解決をスキップするコマンド（`PROJECT_MGMT`）:**
`default`, `help`, `setup`, `presets`
...
> ★ `enrich` は AI が analysis.json の各エントリに `summary` / `detail` / `chapter` / `role` フィールドを一括付与する新ステップ。バッチ処理（デフォルト: 20 エントリ / 3000 行単位）+ 再開可能。
...
| `writeAgentContext()` | `.claude/rules/sdd-*.md` にコンテキストファイルを書き込む |
| `cleanupAgentContext()` | コンテキストファイルを削除する |

- `{{PROMPT}}` プレースホルダーで引数リストにプロンプトを注入（未使用の場合は末尾に追加）
- `ARGV_SIZE_THRESHOLD` = 100,000 バイト：プロンプトがこのサイズを超える場合は stdin 経由で渡す（OS の `ARG_MAX` 制限回避）
...
| `resolveConcurrency(cfg)` | 並列処理数を解決（デフォルト: 5） |
...
| `.sdd-forge/context.json` | プロジェクトコンテキスト文字列（`config.json` の `textFill.projectContext` より優先） |
...
#### `src/docs/commands/enrich.js` — AI エンリッチメント
...
| スキップ条件 | 既に enrich 済みのエントリは再実行時にスキップ。`defaultAgent` 未設定時もスキップ |
...
| `chapters` | — | `string[]` | プロジェクト固有の章順序上書き（preset.json の `chapters` より優先） |
| `agentWorkDir` | — | `string` | AI エージェントの `-C <dir>` 引数に渡す作業ディレクトリパス |
...
│   │                                scan, setup, snapshot, text-batch, text-helpers
...
- **`--system-prompt-file` は存在しない**: Claude CLI にこのオプションはない（過去に誤追加→削除済み）。プロンプトが `ARGV_SIZE_THRESHOLD`（100,000 バイト）を超える場合は自動的に stdin 経由で渡される（`ARG_MAX` 制限回避）。
...
- **enrich バッチ再開**: `enrich.js` は途中で中断しても再実行時に既処理エントリをスキップして継続できる。また `defaultAgent` 未設定時はビルドパイプライン内でスキップされる。
- **章ファイル命名規則**: 番号プレフィックスなし（例: `overview.md`, `cli_commands.md`）が現在の仕様。番号付き（`01_overview.md` 等）は旧フォーマット。
- **chapters 上書き**: `config.json` の `chapters` でプロジェクト固有の章順序を `preset.json` の定義より優先して指定できる。
