 ...
 */
export function createProgress(steps, { verbose = false } = {}) {
  ...
  const instance = {
    ...
    start(label) {
      ...
      if (useBar) {
        updateHeader(renderBar(idx, label + " ...", doneWeight));
      }
    },
    ...
    stepDone() {
      if (currentIdx >= 0 && currentIdx < steps.length) {
        ...
        if (useBar) {
          updateHeader(renderBar(currentIdx, steps[currentIdx].label + " done", doneWeight));
        }
      }
    },
     ...
     */
      ...
      if (useBar) {
        ...
        updateHeader(`[${steps.length}/${steps.length}] ${bar} 100%  done \u2713`);
        process.stderr.write("\n");
      }
      _current = null;
    },
