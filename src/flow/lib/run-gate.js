 ...
 *
 * FlowCommand: gate — check spec readiness before implementation.
 */
...
export class RunGateCommand extends FlowCommand {
  async execute(ctx) {
    ...
    if (issues.length > 0) {
      throw new Error(["spec gate check failed", ...issues].join("\n"));
    }
    ...
    if (!skipGuardrail) {
      ...
      if (result) {
        ...
        if (!result.passed) {
          throw new Error(
            ["spec gate check failed", ...reasons.filter((r) => r.verdict === "FAIL").map((r) => r.detail)].join("\n"),
          );
        }
      }
    }
